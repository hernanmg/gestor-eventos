...
import {
  useSiniestros, useSiniestro, useCreateSiniestro, useUpdateSiniestro, useCerrarSiniestro,
...
import type { EstadoSiniestro, TipoSiniestro, SiniestroEmpleado } from '@/types';
...
const ESTADOS: EstadoSiniestro[] = ['ABIERTO', 'EN_TRAMITE', 'CERRADO', 'RECHAZADO'];

// ── Nuevo siniestro ────────────────────────────────────────────────────────────

function NuevoSiniestroDialog({ onClose }: { onClose: () => void }) {
...
function SiniestroDrawer({ id, canEdit, onClose }: { id: number; canEdit: boolean; onClose: () => void }) {
      ...
      <DialogContent className="sm:max-w-3xl max-h-[85vh] overflow-y-auto">
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-sm bg-muted/20 rounded p-3">
            <p><span className="text-muted-foreground">Tipo:</span> {TIPO_LABEL[siniestro.tipo]}</p>
  ...
  return (
    <div className="space-y-3">
      <p className="text-xs text-muted-foreground bg-muted/30 rounded px-3 py-2">
        🚑 Siniestros de empleados — accidentes de trabajo y cobertura ART. Los siniestros de vehículos (choques, robos, daños) los maneja Flota/Lorena por separado.
      </p>
          ...
          <table className="w-full text-sm">
            <thead className="border-b bg-muted/30">
              <tr>
                ...
                <th className={th}>ART N°</th>
                <th className={th}>Estado</th>
                <th className={th}>Días baja</th>
                <th className={th}>Gasto total</th>
              </tr>
            </thead>
            <tbody className="divide-y">
              {siniestros.length === 0 ? (
                <tr><td colSpan={7} className="px-3 py-6 text-center text-sm text-muted-foreground">Sin siniestros para este filtro.</td></tr>
              ) : siniestros.map((s: SiniestroEmpleado) => (
                <tr key={s.id} className="cursor-pointer hover:bg-muted/10" onClick={() => setViewingId(s.id)}>
                  <td className={td}>{s.empleado?.apellido}, {s.empleado?.nombre}</td>
                  <td className={td}>{TIPO_LABEL[s.tipo]}</td>
                  ...
                  <td className={td}>{s.dias_baja ?? '—'}</td>
                  <td className={cn(td, 'text-right tabular-nums')}>{formatCurrency(s.total_gastos)}</td>
                </tr>
              ))}
            </tbody>
          </table>
